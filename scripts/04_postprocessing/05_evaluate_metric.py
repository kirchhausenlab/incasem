...
def evaluate_metric(
    ...
    if labels_roi != probas_roi:
        raise ValueError(
            f"The labels ROI {labels_roi} does not match the prediction ROI {probas_roi}."
        )
    ...
    try:
        ...
        if get_roi(mask) != probas_roi:
            raise ValueError(
                f"Mask ROI {get_roi(mask)} does not cover predictions {probas_roi}."
            )
    ...
    try:
        ...
        if get_roi(metric_mask) != probas_roi:
            raise ValueError(
                f"Metric mask ROI {get_roi(metric_mask)} does not cover predictions {probas_roi}."
            )
    except (TypeError, KeyError, RuntimeError):
