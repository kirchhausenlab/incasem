...
def convert_to_uint8(array):
    ...
    if np.dtype(dtype) == np.uint32:
        ...
        if array.max() > 255:
            raise ValueError("Array contains integers >255, cannot safely convert to uint8.")
        return array.astype(np.uint8)
    elif np.issubdtype(dtype, np.floating):
        if array.min() < 0.0 or array.max() > 1.0:
            raise ValueError("Array contains floats outside [0,1], cannot safely scale to uint8.")
...
def convert(filename, ds_name, out_path, num_workers):
    ...
    logger.info(f"Processing volume of shape {shape} in {total_blocks} blocks (z-axis partition)")
...
if __name__ == '__main__':
    ...
    parser.add('--dataset', '-d', required=True, help="Name of the dataset in the container")
    parser.add('--out_path', '-o', required=True, help="Output directory for TIFF images")
    parser.add('--num_workers', '-n', type=int, default=16, help="Number of Dask workers")
