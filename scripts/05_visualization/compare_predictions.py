...
def compare_predictions(
    ...
    for s, iterations_s in zip(predictions_series, iterations):
        ...
        with viewer.txn() as s:
            for array, dataset in zip(arrays_series, datasets_series):
                add_layer(context=s, array=array, name=dataset, shader=shader)
    ...
    with viewer.txn() as s:
        for array, dataset in zip(input_arrays, input_datasets):
            add_layer(context=s, array=array, name=dataset)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_file", type=str_rstrip_slash, help="The path to the container to show."
    )
