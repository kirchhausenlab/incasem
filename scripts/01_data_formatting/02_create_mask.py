...
from skimage.morphology import ball, binary_opening, binary_closing, remove_small_holes, remove_small_objects
...
def partition_roi(total_shape, block_size):
    ...
    grid_dims = [(total_shape[d] + block_size[d] - 1) // block_size[d]
                 for d in range(dims)]
    slices = []
...
def create_mask(filename, ds_name, out_ds_name, chunk_shape, min_gray_value, max_gray_value, num_workers):
    """
    ...
    for block in blocks:
        ...
        scheduled.append(
            (block, client.submit(process_mask_block, filename, ds_name, block, min_gray_value, max_gray_value)))
...
def parse_args():
    ...
    p.add('--filename', '-f', required=True,
          help='Input zarr container filename')
    p.add('--dataset', '-d', required=True,
          help='Name of the raw dataset in the zarr container')
    p.add('--out_dataset', '-o', default='volumes/mask',
    ...
    p.add('--min_gray_value', type=int, default=2,
          help='Lower boundary for masking by value')
    p.add('--max_gray_value', type=int, default=180,
