...
def clahe_worker(block, raw, out, clahe_kernel_size, clahe_clip_limit):
    ...
    equalized = Array(equalized, roi=block.read_roi, voxel_size=raw.voxel_size)
    out[block.write_roi] = equalized[block.write_roi]


def equalize_histogram(
    filename, ds_name, out_ds_name, chunk_shape, kernel_size, clip_limit, num_workers
):
    ...
    write_roi = Roi((0, 0, 0), raw.voxel_size * Coordinate(chunk_shape))

    # Add (1,1,1) to avoid division of odd number
    # context = ((raw.voxel_size * kernel_size) + (1,) * raw.voxel_size.dims) / 2
    context = (
        raw.voxel_size * kernel_size + Coordinate((1,) * raw.voxel_size.dims)
    ) / 2
    ...
    task = daisy.Task(
        ...
        process_function=lambda block: clahe_worker(
            block, raw, out, kernel_size, clip_limit
        ),
...
def parse_args():
    ...
    p.add(
        "--num_workers", "-n", type=int, default=20, help="Number of daisy processes."
    )
