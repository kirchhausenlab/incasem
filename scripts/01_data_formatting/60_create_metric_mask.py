...
def create_metric_mask_worker(
    block, labels, out, exclude_voxels_outwards, exclude_voxels_inwards
):
    # load the chunk
...
def create_metric_mask(
    ...
    labels = open_ds(filename, ds_name, mode="r")
    ...
    write_roi = Roi((0, 0, 0), labels.voxel_size * Coordinate(chunk_shape))

    context = labels.voxel_size * max(exclude_voxels_outwards, exclude_voxels_inwards)
