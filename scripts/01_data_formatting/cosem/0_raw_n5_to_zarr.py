...
def parse_args():
    ...
    p.add("-n", "--n5", required=True, type=str, help="n5 dataset to convert")
    p.add("-z", "--zarr", required=True, type=str, help="zarr file to be created")
    p.add(
        "-i",
        "--in_arrays",
        required=True,
    ...
    p.add("--chunk_size", default=256, type=int)
    p.add("--offset", default=[0, 0, 0])
    p.add("--resolution", default=[1, 1, 1])
    p.add("--workers", default=32, type=int)
    p.add("--threads_per_worker", default=3, type=int)
    p.add(
        "--raw_same_bit",
        action="store_true",
