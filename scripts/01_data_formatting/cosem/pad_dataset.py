...
def parse_args():
    ...
    p.add("-d", "--ds", required=True, type=str, help="dataset to pad, eg. volumes/raw")
    p.add(
        "-p",
        "--pad",
        type=int,
    ...
    p.add("-c", "--chunks", default=[256, 256, 256])
    p.add("--num_workers", type=int, default=32)
    p.add("--threads_per_worker", type=int, default=3)
    args = p.parse_args()
    logger.info(f"\n{p.format_values()}")
    return args
