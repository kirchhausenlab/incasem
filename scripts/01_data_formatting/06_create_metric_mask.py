...
def process_metric_block(core_slice, context, filename, ds_name,
                         exclude_voxels_outwards, exclude_voxels_inwards):
    """
    ...
    dilated = ndimage.binary_dilation(data, iterations=exclude_voxels_outwards).astype(np.uint8)
    eroded = ndimage.binary_erosion(data, iterations=exclude_voxels_inwards).astype(np.uint8)
...
def create_metric_mask(filename, ds_name, out_ds_name, chunk_shape,
                       exclude_voxels_outwards, exclude_voxels_inwards, num_workers):
    """
    ...
    logger.info("Processing volume of shape %s in grid %s (%d blocks)",
                global_shape, grid_dims, total_blocks)
    ...
    # Schedule each block.
    scheduled = []
    for idx in tqdm(itertools.product(*(range(n) for n in grid_dims)), total=total_blocks):
        core_slice = tuple(
            slice(idx[d] * chunk_shape[d],
                  min((idx[d] + 1) * chunk_shape[d], global_shape[d]))
            for d in range(ndim)
        )
        future = client.compute(dask.delayed(process_metric_block)(
            core_slice, context, filename, ds_name,
            exclude_voxels_outwards, exclude_voxels_inwards
        ))
...
def parse_args():
    ...
    p.add('--out_dataset', '-o', required=True, help='Name of the new metric mask dataset')
    p.add('--chunk_shape', '-c', nargs='+', type=int, default=[128, 128, 128],
          help='Size of an output chunk in voxels')
    p.add('--exclude_voxels_outwards', type=int, default=4,
          help="Number of voxels to dilate (outwards)")
    p.add('--exclude_voxels_inwards', type=int, default=4,
