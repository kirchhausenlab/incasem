...
def process_block_delayed(core_slice, context, raw, kernel_size, clip_limit):
    ...
    if any(p != (0, 0) for p in pad_width):
        read_data = np.pad(
            read_data, pad_width, mode="constant", constant_values=mean_val
        )

    # Apply CLAHE.
    equalized = equalize_adapthist(
        image=read_data, kernel_size=kernel_size, clip_limit=clip_limit, nbins=256
    )
...
def equalize_histogram(
    ...
    for idx in itertools.product(*(range(n) for n in grid_dims)):
        ...
        delayed_block = dask.delayed(process_block_delayed)(
            core_slice, context, raw, kernel_size, clip_limit
        )
