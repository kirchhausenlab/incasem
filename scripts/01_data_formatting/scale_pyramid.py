...
def snap_to_grid(roi, voxel_size):
    ...
    offset, shape = roi
    new_shape = tuple(
        ((shape[d] + voxel_size[d] - 1) // voxel_size[d]) * voxel_size[d]
...
def downscale_block(in_array, out_array, factor, block_read, block_write):
    ...
    desired = [
        (block_write[d].stop - block_write[d].start) * factor[d] for d in range(dims)
    ]
...
def prepare_ds(
    in_file, ds_name, total_roi, voxel_size, write_size, dtype, num_channels
):
    """
    ...
    for scale_num, scale in enumerate(scales):
        try:
            scale = (
                ...
                else (scale,) * len(prev_voxel_size)
            )
        except Exception:
            scale = (scale,) * len(prev_voxel_size)

        next_voxel_size = tuple(
            prev_voxel_size[d] * scale[d] for d in range(len(prev_voxel_size))
        )
        offset, shape = prev_roi
        next_shape = tuple(
            ((shape[d] + next_voxel_size[d] - 1) // next_voxel_size[d])
            * next_voxel_size[d]
            for d in range(len(shape))
        )
        next_total_roi = (offset, next_shape)
        next_write_size = tuple(
            chunk_shape[d] * next_voxel_size[d] for d in range(len(chunk_shape))
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file", "-f", type=str, required=True, help="The input container"
    )
    parser.add_argument(
        "--ds", "-d", type=str, required=True, help="The name of the dataset"
    )
    parser.add_argument(
        "--scales",
        "-s",
    ...
    parser.add_argument(
        "--num_workers", type=int, default=32, help="Number of Dask workers"
    )
