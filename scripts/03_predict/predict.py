...
def multiple_prediction_setup(_config, run_path, model_, checkpoint):
    ...
    for pred_ds in prediction_datasets:
        pred_setups.append(
            prediction_setup(_config, run_path, model_, checkpoint, pred_ds)
        )
    return pred_setups


def prediction_setup(_config, run_path, model_, checkpoint, pred_dataset):
    pipeline_type = {
...
def remove_context(batch, input_size_voxels, output_size_voxels):
    ...
    context = (gp.Coordinate(input_size_voxels) - gp.Coordinate(output_size_voxels)) / 2
...
def log_metrics(
    _run_dummy, target, prediction_probas, mask, metric_mask, run_path, iteration, mode
):
    ...
    precision_recall = fos.metrics.precision_recall(target, prediction_probas, mask)
    for i, (p, r) in enumerate(precision_recall):
        _run_dummy.log_scalar(f"precision_{i}_{mode}", p, iteration)
        logger.info(f"{mode} | Precision class {i}: {p}")
        _run_dummy.log_scalar(f"recall_{i}_{mode}", r, iteration)
        logger.info(f"{mode} | Recall class {i}: {r}")
