...
class TrainingBaselineWithContext:
    ...
    def _assemble_pipeline(self):
        ...
        context = (self._input_size - self._output_size) // self._voxel_size // 2
        ...
        for sources_p in sources.pipelines:
            p = (
                ...
                + fos.gunpowder.DeepCopyArrays(
                    arrays=[keys["LABELS"]], output_arrays=[keys["BACKGROUND_MASK"]]
                )
                + fos.gunpowder.BinarizeLabels([keys["BACKGROUND_MASK"]])
                + fos.gunpowder.SaveBlockPosition(keys["RAW"], raw_pos)
                ...
                + fos.gunpowder.PadDownstreamOfRandomLocation(
                    keys["RAW"], size=None, value=0.5
                )
                + fos.gunpowder.PadDownstreamOfRandomLocation(
                    keys["LABELS"], size=None, value=0
                )
                # Padding mask with 1s to make sure that shallow blocks are
                # used for training
                + fos.gunpowder.PadDownstreamOfRandomLocation(
                    keys["MASK"], size=None, value=1
                )
                + fos.gunpowder.PadDownstreamOfRandomLocation(
                    keys["METRIC_MASK"], size=None, value=1
                )
        ...
        self.pipeline = self.pipeline + fos.gunpowder.Reject(
            mask=keys["MASK"],
            min_masked=0.75,
        ...
        self.pipeline = self.pipeline + self.downsample
        ...
        self.pipeline = self.pipeline + fos.gunpowder.ToDtype(
            [keys["LABELS"]], dtype="int64"
        )
        ...
        self.pipeline = self.pipeline + self.balance_labels
        ...
        self.pipeline = (
            ...
            + fos.gunpowder.DeepCopy([keys["RAW"], keys["LABELS"], keys["MASK"]])
        ...
        self.pipeline = self.pipeline + self.precache

        # checkpoints in some directory
        checkpoint_dir = os.path.join(self._run_path_prefix, "models", self._run_dir)
        ...
        self.train_node = fos.gunpowder.torch.Train(
            ...
            log_dir=os.path.join(
                self._run_path_prefix, "tensorboard", self._run_dir, "training"
            ),
            log_every=1,
        )
        self.pipeline = self.pipeline + self.train_node
        ...
        self.pipeline = self.pipeline + fos.gunpowder.DeepCopyArrays(
            arrays=[keys["RAW"]], output_arrays=[keys["RAW_OUTPUT_SIZE"]]
        )
        self.snapshot = fos.gunpowder.Snapshot(
            dataset_names={v: f"{k.lower()}" for k, v in keys.items()},
            every=1,
            output_dir=os.path.join(
                self._run_path_prefix, "snapshots", self._run_dir, "training"
            ),
