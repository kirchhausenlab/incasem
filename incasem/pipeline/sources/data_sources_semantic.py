...
class DataSourcesSemantic(DataSourcesBase):
    def _assemble_pipeline(
        self, attributes, file_path, key_suffix, voxel_size, roi, raw_roi=None
    ):
        datasets_gp = {}
        array_specs = {}
        ...
        if "metric_masks" in attributes:
            ...
            logger.info(f"No metric mask given, add dummy metric mask of all 1s.")
        ...
        pipeline = gp.ZarrSource(
            file_path, datasets=datasets_gp, array_specs=array_specs
        )
        for key, _ in array_specs.items():
            ...
            pipeline = pipeline + gp.Crop(key, roi_to_crop)

        if "labels" in attributes:
            pipeline = pipeline + BinarizeLabels(array_keys_labels)

            pipeline = pipeline + MergeLabels(
                classes=classes, output_array=self._keys["LABELS"]
            )
        else:
            pipeline = pipeline + AddBackgroundLabels(
                raw_array=self._keys["RAW"], output_array=self._keys["LABELS"]
            )

        if "mask" in attributes:
            pipeline = pipeline + BinarizeLabels([self._keys["MASK"]])
        else:
            pipeline = (
                pipeline
        ...
        if "metric_masks" in attributes:
            pipeline = (
                ...
                + MergeMasks(array_keys_metric_mask, self._keys["METRIC_MASK"])
            )
        else:
            pipeline = (
                pipeline
        ...
        pipeline = pipeline + gp.Normalize(self._keys["RAW"])
