...
class LsdLoss(torch.nn.Module):
    def __init__(self, weight=None, device="cuda"):
        ...
        task_loss = torch.nn.CrossEntropyLoss(reduction="none", weight=self.weight)
    ...
    def forward(
        self, input_task, input_lsd, target_task, target_lsd, mask=None, scaling=None
    ):
        """forward.
        ...
        logger.debug(f"{loss_per_voxel.shape=}")
        logger.debug(f"Scaled loss per elem sum={float(loss_per_voxel.sum())}")
