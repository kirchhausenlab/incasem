...
def determine_roi(image_paths, offset=(0, 0, 0), shape=(None, None, None)):
    ...
    if shape[0]:
        assert offset[0] + shape[0] <= z_shape, "desired z shape exceeds data shape"
        z_shape = shape[0]

    y_shape = img_array.shape[0]
    if shape[1]:
        assert offset[1] + shape[1] <= y_shape, "desired y shape exceeds data shape"
        y_shape = shape[1]

    x_shape = img_array.shape[1]
    if shape[2]:
        assert offset[2] + shape[2] <= x_shape, "desired x shape exceeds data shape"
        x_shape = shape[2]

    shape_cropped = Coordinate([z_shape, y_shape, x_shape])

    roi = Roi(offset, shape_cropped)
...
def write_to_zarr(
    block, voxel_size, image_paths, dataset, dtype, conversion=None, invert=False
):
    ...
    for idx, i in enumerate(paths_to_load):
        ...
        img = img.crop((offset_index[2], offset_index[1], end_index[2], end_index[1]))
        if conversion:
            img = conversion(img)
        # array has shape y, x
        array = np.array(img)
...
def image_sequence_to_zarr(
    ...
    if conversion:
        conversion = globals()[conversion](dtype=dtype, input_path=osp.abspath(raw_dir))
    ...
    block_roi = Roi((0, 0, 0), voxel_size * Coordinate(chunks))
    ...
    task = daisy.Task(
        ...
        process_function=lambda block: write_to_zarr(
            block, voxel_size, image_paths, ds, dtype, conversion, invert=invert
        ),
        fit="shrink",
