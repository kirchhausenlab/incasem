...
def make_config_janelia_crops(zarr_ds: str, organelles: list):
    ...
    for org in organelles:
        crop_folder = os.path.join(zarr_ds, "volumes", "labels", org)
        ...
        for crop in os.listdir():
            ...
            if org in ["mito", "er", "golgi"]:
                config[section]["metric_masks"] = [
                    "volumes/metric_masks/" + org + "/" + crop
                ]

        config_path = os.path.join(os.path.dirname(zarr_ds), org + "_config.json")
...
def make_metric_masks_janelia_crops(zarr_ds: str, organelles: list):
    zarr_ds = zarr_ds.rstrip("\\")  # remove trailing backslash if present
    ...
    for org in organelles:
        ...
        for crop in os.listdir():
            ...
            crop_ds = os.path.join("volumes", "labels", org, crop)

            metric_mask_ds = os.path.join("volumes", "metric_masks", org, crop)
            ...
            os.system(
                f"python ~/code/incasem/scripts/01_data_formatting/60_create_metric_mask.py -f {zarr_ds} -d {crop_ds} --out_dataset {metric_mask_ds} --exclude_voxels_inwards {voxel_to_exclude} --exclude_voxels_outwards {voxel_to_exclude}"
            )
...
def get_crops_coords(zarr_ds: str):
    ...
    label_path = os.path.join(zarr_ds, "volumes", "labels")
    os.chdir(label_path)
    for org in os.listdir():
        ...
        for crop in os.listdir():
            ...
            offset = [
                int(offset_nm[0] / resolution[0]),
                int(offset_nm[1] / resolution[1]),
                int(offset_nm[2] / resolution[2]),
            ]  # z, y, x
...
def make_empty_like_raw(zarr_ds: str, array_to_make: str):
    ...
    array_path = fos.utils.create_zarr(
        zarr_ds,
        array_to_make,
        shape=raw_shape,
    ...
    shutil.copy(os.path.join(raw_path, ".zattrs"), os.path.join(array_path, ".zattrs"))
...
def make_mask_janelia_crops(zarr_ds: str):
    ...
    mask = da.from_array(mask_arr, chunks=mask_chunks, asarray=True)

    crops_offset, crops_shape = get_crops_coords(zarr_ds)

    for n in range(len(crops_offset)):
        mask[
            crops_offset[n][0] : crops_offset[n][0] + crops_shape[n][0],
            crops_offset[n][1] : crops_offset[n][1] + crops_shape[n][1],
            crops_offset[n][2] : crops_offset[n][2] + crops_shape[n][2],
        ] = 255
