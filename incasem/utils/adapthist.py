...
def equalize_adapthist(image, kernel_size=None, clip_limit=0.01, nbins=256):
    ...
    * For RGBA images, the original alpha channel is removed.
    ...
    image = np.round(
        rescale_intensity(image, in_range="dtype", out_range=(0, NR_OF_GRAY - 1))
    ).astype(np.uint16)
...
def _clahe(image, kernel_size, clip_limit, nbins):
    ...
    hist = map_histogram(hist, 0, NR_OF_GRAY - 1, np.prod(kernel_size))
    ...
    blocks = np.reshape(blocks, (np.prod(ns_proc), np.prod(blocks.shape[ndim:])))
