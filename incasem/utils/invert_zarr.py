...
def invert_zarr(zarr_ds: str, chunks: list, workers=20, threads_per_worker=3):
    ...
    inv_path, _, _, chunks, _ = fos.utils.make_empty_like_raw(
        zarr_ds, "volumes/raw_inverted"
    )

    #  fill it with 255 where there are crops
    inv = read(inv_path)
    inv_data = da.from_array(inv, chunks=chunks, asarray=True)

    raw = read(os.path.join(zarr_ds, "volumes/raw"))
    raw_data = da.from_array(raw, chunks=chunks, asarray=True)
