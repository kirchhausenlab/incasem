...
def match_histograms_masked(source, template, source_mask, template_mask):
    ...
    """
    src_values, src_unique_indices = np.unique(source.ravel(), return_inverse=True)

    source_masked = source[source_mask.astype(np.bool)]
    src_values_masked, src_counts = np.unique(source_masked.ravel(), return_counts=True)

    assert np.all(src_values == src_values_masked)

    template_masked = template[template_mask.astype(np.bool)]
    tmpl_values, tmpl_counts = np.unique(template_masked.ravel(), return_counts=True)
