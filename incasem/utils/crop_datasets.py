...
def add_shift_to_roi(roi, shift):
    ...
    """
    return tuple(slice(s.start + shift[i], s.stop + shift[i]) for i, s in enumerate(roi))
...
def prepare_new_dataset(zarr_filename, ds_name, total_roi, voxel_size, write_size, dtype, compressor):
    """
...
def partition_roi(total_shape, block_shape):
    ...
    grid_dims = [(total_shape[d] + block_shape[d] - 1) // block_shape[d] for d in range(dims)]
    blocks = []
    for idx in itertools.product(*(range(n) for n in grid_dims)):
        block = tuple(
            slice(idx[d] * block_shape[d], min((idx[d] + 1) * block_shape[d], total_shape[d]))
            for d in range(dims)
        )
        blocks.append(block)
...
def crop_dataset_worker(block, in_ds, out_ds, read_shift):
    ...
    out_ds[block.write_roi] = data[tuple(slice(0, d) for d in roi_shape(block.write_roi))]
    return 0


# --- Main Function: crop_dataset ---
def crop_dataset(zarr_filename, ds_name, out_filename, out_ds_name,
                 crop_offset_voxels, crop_shape_voxels, chunk_shape, dtype, num_workers):
    """
    Crop a dataset from a Zarr container.
    ...
    shifted_roi = (tuple(crop_offset[i] - shift_to_origin[i] for i in range(len(crop_offset))), crop_shape)
    logger.debug(f"Shifted ROI: {shifted_roi}")

    out_ds = prepare_new_dataset(out_filename, out_ds_name, shifted_roi, voxel_size, tuple(chunk_shape), dtype,
    ...
    scheduled = []
    for block in blocks:
        scheduled.append((block, client.submit(crop_dataset_worker, block, in_ds, out_ds, shift_to_origin)))
...
def crop_datasets(zarr_filename, out_filename, datasets, out_datasets, offset_voxels, shape_voxels, chunk_shape, dtypes,
                  num_workers):
    """
    ...
    """
    assert len(datasets) == len(out_datasets), "Provide one out_dataset for each dataset."
    assert len(datasets) == len(dtypes), "Provide one dtype for each dataset."

    for ds, out_ds, dt in zip(datasets, out_datasets, dtypes):
        crop_dataset(zarr_filename, ds, out_filename, out_ds, offset_voxels, shape_voxels, chunk_shape, dt, num_workers)
