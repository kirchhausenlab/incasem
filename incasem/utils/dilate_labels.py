...
def dilate_labels(labels, iterations, dtype, ambiguous_labels="max", background=0):
    ...
    for lab in np.unique(labels):
        ...
        dilated_mask = ndimage.binary_dilation(mask, iterations=iterations).astype(
            dtype
        )

        dilated_instance = dilated_mask * lab
        dilated = __merge_labels(dilated_instance, dilated, ambiguous_labels, max_label)
...
def __merge_labels(source, target, ambiguous_labels, max_label):
    if ambiguous_labels == "max":
        ...
        raise ValueError(f"Invalid ambiguous labels strategy {ambiguous_labels}")
