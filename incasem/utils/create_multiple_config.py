...
def create_multiple_config(config_file):
    ...
    """
    # /nfs/scratch/ajain/incasem/scripts/03_predict/data_configs/test_regions.json
    with open(os.path.expanduser(config_file), "r") as f:
        config = json.load(f)

        datasets = []
        for idx, key in enumerate(config):
            numbered_path = os.path.expanduser(config_file).replace(
                ".json", f"_{idx}.json"
            )

            path_list = numbered_path.split("/")

            temp_folder = os.path.join("/".join(path_list[:-1]), "temp")

            if not os.path.exists(temp_folder):
                os.makedirs(temp_folder)

            temp_path = os.path.join(temp_folder, path_list[-1])
