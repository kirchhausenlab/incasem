...
def gt_to_organelle(
    ...
    #
    gt_base_folder = os.path.join(zarr_ds, "volumes", "groundtruth")
...
def create_label(
    ...
    gt_array = read(gt_ds)
    data = da.from_array(gt_array, chunks=chunks, asarray=True)

    labels = gt_to_labels(data, organelles)

    for organelle in labels:  # mito, golgi, er, background
        zarr_arr_path = os.path.join("volumes", "labels", organelle, ds)
        ...
        zarr_data = da.from_array(data_to_save, chunks=chunks, asarray=True)

        zarr_ds_path = fos.utils.create_zarr(
            zarr_ds,
            zarr_arr_path,
...
def gt_to_labels(gt, organelles):
    ...
    gt_to_label_map = {"mito": [3, 4], "golgi": [6, 7], "er": [16, 17], "np": [22, 23]}
    label_ds = {}
    ...
    for org in organelles:
        ...
        if org_in_gt:
            label_ds[org] = np.where(
                np.logical_or(
                    gt_np == gt_to_label_map[org][0], gt_np == gt_to_label_map[org][1]
                ),
                255,
                0,
            ).astype("u1")
