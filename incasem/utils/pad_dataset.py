...
def pad_dataset(
    zarr: str, ds: str, pad: list, chunks: list, workers=20, threads_per_worker=3
):
    ...
    padded_path = fos.utils.create_zarr(
        zarr,
        padded_ds,
        shape=padded_shape,
    ...
    padded_data = da.from_array(padded, chunks=chunks, asarray=True)

    original = read(ds_path)
    original_data = da.from_array(original, chunks=chunks, asarray=True)

    pad_to_apply = (
        (pad[0], pad[1]),  # pad before and after this axis
        (pad[2], pad[3]),
        (pad[4], pad[5]),
    )
