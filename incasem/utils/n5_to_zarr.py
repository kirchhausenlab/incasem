...
def n5_to_zarr_gt(
    ...
    for arr_in, arr_out in arrays:
        raw_shape = shape_raw_from_meta(
            os.path.join(zarr_path, "volumes", "raw", ".zarray")
        )
...
def metadata_n5_to_zarr(n5_arr: str, zarr_arr: str, resolution: list, raw_shape: list):
    res_x = resolution[0]
    res_y = resolution[1]
    res_z = resolution[2]
    ...
    with open(n5_metadata, "r") as infile:
        ...
        if "jurkat" in n5_arr:
            y_offset = 10
        zarr_attrs["offset"] = [
            int(n5_meta["offset"][2] * 0.25),
            raw_shape[1]
            - int(n5_meta["offset"][1] * 0.25)
...
def dataset_n5_to_zarr(
    ...
    data = da.from_array(
        n5_arr, chunks=(chunk_size, chunk_size, chunk_size), asarray=True
    )
...
def crops_paths(
    ...
    crops_names = [
        c for c in os.listdir(crops_base_dir) if "crop" in c
    ]  # 'Crop3', 'Crop25'..
    ...
    for crop_name in crops_names:
        crops_arrs.append(os.path.join(n5_prefix, crop_name, n5_suffix))

        dest_arrs.append(os.path.join(zarr_suffix, crop_name.replace("crop", "")))
