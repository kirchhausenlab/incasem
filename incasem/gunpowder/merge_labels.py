...
class MergeLabels(gp.BatchFilter):
    ...
    def __init__(
        ...
        assert self.ambiguous_labels in ("max", "background", "preference")
    ...
    def process(self, batch, request):
        ...
        if self.ambiguous_labels == "max":
            ...
            sum_of_binaries = np.sum(
                np.array([batch[array].data for array, _ in self.classes.items()]),
                axis=0,
            )
            ...
            not_ambiguous = np.logical_and(labels_mask, np.logical_not(ambiguous))
